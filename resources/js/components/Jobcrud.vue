  ...
  >
    <DxEditing mode="row" :use-icons="true"> </DxEditing>
    ...
    <DxColumn caption="Action" cell-template="buttondropdown" type="buttons"> </DxColumn>
    <template #customButtonTemplate="{ data }">
       <DxButton
        @click="checkItem(data.row.data.id)"
        text="Duplicate"
      ></DxButton>
       <!-- <template #customButtonTemplate="{ data }">
  <div>
    ...
    >
      <DxButtonIcon icon="dx-icon-duplicate"></DxButtonIcon>
      <DxButtonItem text="Duplicate" @click="duplicateItem(data.row.data.id)"></DxButtonItem>
    </DxDropDownButton>
  </div>  -->
</template>
...
export default {
  ...
  components: {
    ...
    masterTemplate,DxDropDownButton
  ...
  setup() {
    ...
    const checkItem = async (id) => {
      try {
        const response = await axios.post(`/jobs/draft/${id}`);
