  ...
  <div>
      ...
      <v-row v-for="(education, index) in educationDetails" :key="index">
          ...
          <v-col v-if="index > 0" md="2">
              <v-btn @click="removeEducationEntry(index)" color="red" class="custom-button"><v-icon>mdi-trash-can-outline </v-icon></v-btn>
          </v-col>
          <v-col md="2" v-else>
              <v-btn @click="addEducationEntry" color="blue" class="custom-button">  <v-icon>mdi-plus</v-icon></v-btn>
          </v-col>
      </v-row>
  </div>
