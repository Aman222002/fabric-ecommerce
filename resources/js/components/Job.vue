<template>
  <div class="full-screen-image-container">
    ...
    <v-img src="/storage/assest/1.png" contain class="mb-4 full-screen-image"></v-img>
  ...
  <v-dialog v-model="jobDialog" max-width="600px" @click:outside="jobDialog = false" class="login-dialog">
    <v-card>
      <v-card-title style="font-size: 30px">Login</v-card-title>
      <v-card-text>

        <v-text-field label="Company Email" v-model="formData.email" variant="outlined" :rules="emailRules" outlined
          required style="font-size: 20px; font-weight: bold"></v-text-field><br>
        <v-text-field label="Password" variant="outlined" v-model="formData.password" outlined required
          :rules="companyNameRules" type="password" style="font-size: 20px; font-weight: bold"></v-text-field>
      </v-card-text>
